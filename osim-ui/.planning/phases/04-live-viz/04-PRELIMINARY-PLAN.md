# Phase 4 — Live-Visualisierung mit GraphObject-Vollausbau

**Milestone:** v0.1.0
**Stand:** 2026-05-20 (Entwurf, umgenummert auf Phase 4 nach Roadmap-Resync 2026-05-21)
**Aufwand-Schätzung:** 3–4 Wochen (1 Dev)
**Voraussetzungen:** Phase 1 (GraphObject-Basis aus Durchlaufplan-Design-Viewer), Phase 2 (Sim-Lauf als Datenquelle), Phase 3 (JSON-Modell-Format für Live-Konsum).
...
│
├── osim/
│   ├── nodes/                        # Durchlaufplan-Knoten (Phase 4 Welle 2)
...
│   ├── edges/                        # Durchlaufplan-Kanten (Phase 4 Welle 2)
│   │   ├── PDlplKante.tsx          # Standard-Kante
│   │   ├── PDpKaUebergang.tsx      # Übergang
│   │   └── PDpKaExtern.tsx         # Externer Übergang
│   ├── viewers/                      # Phase 6+ und Phase 8+ bauen hier weiter
│   │   ├── (PDurchlaufplanViewer.tsx — eigentlich Phase 4, einfacher Konsument)
│   │   ├── (PRessBelegMatrixViewer.tsx — Phase 8+ Backlog)
│   │   ├── (AEinsatzWunschViewer.tsx — Phase 8+ Backlog)
│   │   ├── (PSimulatorViewerGfx.tsx — Phase 4, das Live-Dashboard-Element)
│   │   └── (PAuslastungsChartViewer.tsx — Phase 6 Reports)
│   └── animation/
...
| ID | Task | Deps |
|---|---|---|
| 4.1 | `persistence/serializer.ts`: GraphView ↔ JSON (Schema gemeinsam mit Phase 3) | Phase 3 |
...
| 7.4 | Doku in `docs/PHASE-4-GRAPH-OBJECT.md` (Architektur + API) | alle |
...
| WebSocket-Multiplexing in API-Process skaliert nicht über Replicas | Phase 4 nur in-Process; Cloud-fähig wird es in Phase 5 (Pub/Sub) |
...
## 7. Was Phase 4 NICHT liefert

- Modell-Editor mit Drag-and-Drop neuer Knoten aus einer Palette (→ Phase 8+ Backlog, optional)
- DAG-Pläne mit zyklischen Strukturen (Engine V2)
- Cloud-skalierende Live-Übertragung (→ Phase 5 mit Pub/Sub)
- Wiedergabe alter Läufe (Replay-Modus) — nur Live; Replay optional Phase 6
- Animations-Geschwindigkeit-Slider wie im Original ("Event Block Size") — Phase 4 zeigt einfach 1:1 was kommt
...
5. `docs/PHASE-4-GRAPH-OBJECT.md` erklärt API + zeigt Beispiel-Subklasse
6. Demo-Video: Live-Lauf eines `Fertigungsstruktur1`-Modells
7. **Foundation-Bereitschaft:** API-Stabilität so, dass Phase 6 (Reports) und Phase 8+ (Matrix-/Gantt-Viewer im Vollausbau) ohne Bruch konsumieren können. Mindestens 1 nicht-Durchlaufplan-Beispiel als Proof-of-Concept (z. B. ein einfacher Matrix-Viewer-Stub via GraphGrid).
